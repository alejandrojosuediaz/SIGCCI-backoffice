...
module.exports = ({ strapi }) => ({
  ...
  async getDocumentSnapshot(uid, params = {}) {
    try {
      if (!uid || (!params?.documentId && !params?.id)) {
        return null;
      }
      ...
      const snapshot = await strapi.documents(uid).findOne({
        ...query,
        populate: "*",
      });
      ...
      strapi.log.warn(`Could not fetch previous data for ${uid}: ${error.message}`);
      return null;
    }
  },
  ...
  getChangedFields(previousData = {}, newData = {}) {
    const previous = previousData && typeof previousData === "object" ? previousData : {};
    const current = newData && typeof newData === "object" ? newData : {};
    const keys = Array.from(new Set([...Object.keys(previous), ...Object.keys(current)]));

    const excludedFields = new Set(["updatedAt", "publishedAt", "updated_by_id"]);
    ...
    const changedFields = keys
      ...
      .map((key) => ({
        field: key,
        oldValue: this.normalizeAuditValue(previous[key]),
        newValue: this.normalizeAuditValue(current[key]),
      }));

    return {
      changedFields,
      oldData: this.normalizeAuditValue(previous),
      newData: this.normalizeAuditValue(current),
    };
  },
  ...
  initializeEventListeners() {
    ...
    strapi.documents.use(async (context, next) => {
      ...
      if (operation === "update") {
        previousData = await this.getDocumentSnapshot(uid, context.params || {});
      }
      ...
      setTimeout(async () => {
        try {
          ...
          const updateDiff =
            operation === "update" ? this.getChangedFields(previousData, result) : null;
          ...
          const logData = {
            ...
            data:
              operation === "update"
                ? {
                    changedFields: updateDiff?.changedFields || [],
                    oldData: updateDiff?.oldData || {},
                    newData: updateDiff?.newData || {},
                  }
                : result,
          };
