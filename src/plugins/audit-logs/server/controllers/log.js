  ...
  async find(ctx) {
    try {
      ...
      if (query.user && typeof query.user === "string") {
        ...
        if (userSearch) {
          filters.$or = [
            { userDisplayName: { $containsi: userSearch } },
            { userEmail: { $containsi: userSearch } },
            { user: { username: { $containsi: userSearch } } },
            { user: { firstname: { $containsi: userSearch } } },
            { user: { lastname: { $containsi: userSearch } } },
          ];
        }
      }
